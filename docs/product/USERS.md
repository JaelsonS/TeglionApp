...
> Este documento consolida conteúdo antes espalhado em `docs/01-ESTRATEGIA/PUBLICO-ALVO.md` e `docs/01-ESTRATEGIA/PROBLEMAS-QUE-RESOLVE.md` (arquivos removidos nesta reorganização de 19/08/2026).
...
Dentro do escritório, dois papéis usam o Teglion de formas diferentes:
...
O produto assume um usuário que não é técnico, mas também não é resistente a usar um sistema web no lugar de planilha e WhatsApp — é o perfil de quem já usa email, já usa alguma ferramenta de faturamento obrigatória por lei, e está disposto a trocar processo manual por processo mais rápido, desde que a curva de aprendizado seja curta. O fluxo de convite simples (o escritório convida, o cliente aceita e já está dentro) reflete essa aposta: sem etapa de configuração complexa do lado do cliente.

## Quem não é o público hoje

Escritórios grandes, com departamentos formais e processos de compra corporativos, não são o alvo da fase atual — o produto não tem hoje as camadas de permissão granular, relatório gerencial ou integrações que esse porte normalmente exige. Também não é uma aposta em usuário final individual sem vínculo com um escritório: o Teglion não vende diretamente para a pessoa física, ela entra através do escritório que a atende.
...
**Zero visibilidade sobre a própria operação.** Sem um sistema central, o dono do escritório não tem como responder rápido "quantos clientes tenho pendente de documento essa semana" ou "quantas mensagens ainda não respondi". O Teglion junta isso numa visão única em vez de depender da memória de quem está no dia a dia.

## Para onde isso aponta

O público de escritórios pequenos/médios em Portugal é o ponto de partida deliberado, não o teto. A extensão natural — outros tipos de serviço profissional com a mesma dinâmica de cliente recorrente, prazo e documento, e outros mercados de língua portuguesa — está discutida em [VISION.md](./VISION.md).
