...
| 4 | MFA (TOTP, recovery, owner obrigatório) | `EM ANDAMENTO` — plano técnico aprovado (21/08/2026); implementação em `feature/fase-4-mfa` (staging-first). **Não concluída** até Security Gate + UAT. |
...
- **Fase 4 — MFA: `CONCLUÍDO` tecnicamente; UAT ao vivo pendente.** TOTP implementado e mergeado em staging (`feature/fase-4-mfa` → `staging`, 21/08/2026): enrollment, challenge, recovery codes, owner obrigatório/staff opcional, refresh/SSO gate. Reforçado numa sessão de hardening (24/08/2026): challenge JWT ganhou protecção anti-replay (jti one-time), código TOTP passou a ser de uso único por-utilizador em qualquer contexto (login, acções sensíveis, cofre), e a chave do rate-limit deixou de poder ser multiplicada gerando challenges novos. Staging-first; sem produção/Cloudflare/Vercel/DNS/`main`. Incidente de schema MFA órfão em produção: `docs/security/MFA_PROD_SCHEMA_INCIDENT.md` (colunas inertes, `mfa_enabled=false` em todas as linhas — não corrigido, decisão deliberada). UAT autenticado ao vivo continua pendente (precisa de operador com conta real).
