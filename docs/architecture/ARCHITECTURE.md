...
O Teglion é um monólito modular: um backend Express único, organizado em módulos de domínio, servindo uma aplicação React única. Não é microsserviços — decisão consciente, não limitação. Com o tamanho de time e de produto de hoje (4 escritórios pilotos), separar em serviços distintos criaria custo de coordenação sem benefício real. A modularidade que importa agora acontece dentro do próprio backend: cada domínio de negócio (cliente, documento, agendamento, cobrança, integração) vive em seu próprio módulo, com fronteira clara, mesmo rodando no mesmo processo.
...
Frontend e backend são deploys separados, que só se falam por HTTP. O frontend nunca acessa o Supabase diretamente — toda leitura e escrita de dado passa pelo backend. Não existe, em nenhum ponto do código do frontend, uma chamada direta ao Supabase; só chamadas à própria API do Teglion, através da camada `infrastructure/` (ver [FRONTEND.md](./FRONTEND.md)).

## Como as camadas se encaixam

Dentro de um módulo de backend, o caminho é sempre o mesmo:
...
- **Controller** lida só com a borda HTTP: lê parâmetros, valida forma de entrada, monta a resposta. Não tem regra de negócio.
- **Service** concentra a regra de negócio do domínio.
- **Repository** é a única camada que sabe como o dado é lido ou gravado no Supabase — nenhum controller ou service monta consulta ao banco diretamente.

Essa separação é consistente nos módulos do backend (`backend/src/modules/*`), cada um com o próprio conjunto de rota, controller, service e repository.
...
1. **Frontend.** O React SPA (rodando no navegador, servido pela Vercel) dispara uma chamada HTTP para a API do backend — por exemplo, `GET /api/v1/contabil/clients` — através da camada `infrastructure/` do frontend. Não há acesso direto ao Supabase em nenhum ponto do frontend.
...
4. **Contexto de tenant.** Para rotas do módulo contábil, `requireActiveFirm` (`backend/src/middlewares/firm-access.middleware.js`) lê o `firmId` da sessão, busca o registro do escritório e verifica o status (ativo, em teste, suspenso). Se o escritório não estiver em condição de uso, a requisição para aqui — antes de chegar a qualquer controller. O detalhe completo de como o isolamento entre escritórios é garantido está em [MULTI_TENANCY.md](./MULTI_TENANCY.md).
...
7. **Banco.** A conexão ao Supabase Postgres usa a `service_role` — uma chave que ignora as políticas de Row Level Security (RLS). Isso significa que a proteção de isolamento entre escritórios, nessa consulta, não vem do banco: vem inteiramente da disciplina de sempre incluir `firm_id` na camada de repositório. Esse ponto é central o suficiente para merecer um documento próprio — [MULTI_TENANCY.md](./MULTI_TENANCY.md).
8. **Resposta.** O resultado sobe de volta: repository → service → controller → JSON para o frontend. Se algo falhar em qualquer ponto do caminho, passa pelo handler central de erro, que padroniza a resposta e evita vazar detalhe interno sensível (ver [BACKEND.md](./BACKEND.md)).
