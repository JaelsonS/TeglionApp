  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        sideOffset={10}
        className="w-[min(100vw-1.5rem,24rem)] overflow-hidden rounded-2xl border-border/70 p-0 shadow-xl"
      >
        ...
        <ul className="max-h-[min(28rem,70dvh)] overflow-y-auto">
          {isLoading ? (
            <li className="px-4 py-10 text-center text-sm text-muted-foreground">A carregar…</li>
          ) : items.length === 0 ? (
            <li className="px-4 py-10 text-center text-sm text-muted-foreground">Sem notificações</li>
          ) : (
            items.map((n) => (
