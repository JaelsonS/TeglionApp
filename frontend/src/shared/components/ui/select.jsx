...
const triggerClassTokens = [
  "flex", "h-9", "w-full", "items-center", "justify-between", "whitespace-nowrap",
  "rounded-md", "border", "border-input", "bg-background", "px-3", "py-2", "text-sm",
  "text-foreground", "shadow-sm", "ring-offset-background", "data-[placeholder]:text-muted-foreground",
  "focus:outline-none", "focus:ring-1", "focus:ring-ring", "disabled:cursor-not-allowed",
  "disabled:opacity-50", "dark:bg-card", "dark:text-foreground",
  "dark:data-[placeholder]:text-slate-300", "[&>span]:line-clamp-1",
]
...
const contentPopperClassTokens = [
  "data-[side=bottom]:translate-y-1", "data-[side=left]:-translate-x-1",
  "data-[side=right]:translate-x-1", "data-[side=top]:-translate-y-1",
]

const viewportPopperClassTokens = [
...
const SelectContent = React.forwardRef(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        contentClassTokens.join(" "),
        position === "popper" &&
          contentPopperClassTokens.join(" "),
        className
      )}
      position={position}
      {...props}>
      ...
      <SelectPrimitive.Viewport
        className={cn("p-1", position === "popper" &&
          viewportPopperClassTokens.join(" "))}>
        {children}
      </SelectPrimitive.Viewport>
